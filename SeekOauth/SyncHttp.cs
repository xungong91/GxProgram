    ...
    public class SyncHttp
    {
        ...
        public string HttpPostWithFile(string url, string queryString, List<Parameter> files)
        {
            ...
            try
            {
                ...
                string fthformdataTemplate = "--" + boundary + "\r\nContent-Disposition: form-data; name=\"{0}\"\r\n\r\n{1}";
                ...
                foreach (Parameter param in listParams)
                {
                    ...
                    if (j == 0)
                    {
                        formitem = string.Format(fthformdataTemplate, param.Name, param.Value);
                    }
                    else
                    {
                        formitem = string.Format(formdataTemplate, param.Name, param.Value);
                    }
                    j++;
                    
                    byte[] formitembytes = Encoding.GetEncoding("GBK").GetBytes(formitem);
