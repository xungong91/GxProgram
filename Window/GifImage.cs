...
namespace Test
{
    ...
    public class GifImage : System.Windows.Controls.UserControl
    {
        ...
        private void CreateNonGifAnimationImage()
        {
            ...
            try
            {
                src = (ImageSource)(new ImageSourceConverter().ConvertFromString(Source));
            }
            catch
            {
                src = (ImageSource)(new ImageSourceConverter().ConvertFromString(AppDomain.CurrentDomain.BaseDirectory + "Image\\ImageError.jpg"));
            }
            image.Source = src;
